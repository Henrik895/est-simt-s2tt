...
class WhisperAgent(SpeechToTextAgent):
    def __init__(self, args = None):
        ...
        print("WhisperAgent device:", self.device)
        print("FP16 enabled:", self.fp16_enabled)
    ...
    def policy(self, states: Optional[AgentStates] = None):
        ...
        # Use previous output as a prefix, where generation continues
        previous_text = " ".join(list(map(lambda x: x.content, self.states.whisper_segments)))
...
class NLLBAgent(TextToTextAgent):
    ...
    def policy(self, states: Optional[AgentStates] = None):
        ...
        translation_decoder_ids = states.decoder_ids.to(self.device) # Continue from previous step
        ...
        # The last word may not be complete, so remove it.
        if not states.source_finished and len(prediction) > 1:
            prediction = prediction[:-1]

        states.prefix = states.prefix + prediction
        ...
        # The last token is usually end of sentence token
        tokenized = tokenized[:,:-1]
        # The beginning of the sentence token goes missing when using tokenizer,
        # so put it back at the beginning. Otherwise the model generates
        # seemingly random tokens at next step.
        tokenized = torch.cat((torch.tensor([[2]]), tokenized), dim=1) 
...
class CascadedPipeline(AgentPipeline):
    pipeline = [WhisperAgent, NLLBAgent]
